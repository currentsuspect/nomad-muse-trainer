...
- Train/val/test stratified splitting (with safe fallback)
...
class MIDITokenizer:
    ...
    def tokenize_midi(self, midi_path):
        ...
        # Extract notes from all instruments (skip drums for now)
        all_notes = []
...
def prepare_dataset(
    ...
    logger.info(
        f"Processed {len(all_sequences)} valid sequences (skipped {skipped} corrupted/invalid files)"
    )
    ...
    # -----------------------------
    # SAFE STRATIFIED SPLIT (FIX)
    # -----------------------------
    buckets_np = np.array(buckets, dtype=np.int32)
    _, bucket_counts = np.unique(buckets_np, return_counts=True)

    stratify_train = buckets_np if bucket_counts.min() >= 2 else None
    if stratify_train is None:
        logger.warning(
            f"Stratified split disabled: least populated bucket has {bucket_counts.min()} sample(s). "
    ...
    if stratify_train is not None:
        temp_buckets = buckets_np[temp_idx]
        _, temp_counts = np.unique(temp_buckets, return_counts=True)
        stratify_temp = temp_buckets if temp_counts.min() >= 2 else None
        if stratify_temp is None:
            logger.warning(
                f"Temp stratified split disabled: least populated temp bucket has {temp_counts.min()} sample(s). "
                "Falling back to random split for val/test."
            )
    else:
        stratify_temp = None
    ...
    stats = {
        ...
        "train_chunks": int(len(train_data)),
        "val_chunks": int(len(val_data)),
        "test_chunks": int(len(test_data)),
    ...
    with open(stats_path, "w") as f:
        json.dump(stats, f, indent=2, default=lambda o: int(o) if isinstance(o, (np.integer,)) else float(o) if isinstance(o, (np.floating,)) else str(o))
